#!/bin/bash

. $(dirname ${BASH_SOURCE[0]})/script-parameters.sh
. $(dirname ${BASH_SOURCE[0]})/script-parameters.local.sh

echo -e "${LIGHT_GREEN}Without drush alias, change temporarily directory to www.${NC}"
cd $WEB_PATH

. $SCRIPTS_PATH/tasks/database_backup.sh

echo -e "${LIGHT_GREEN}Put the site in maintenance mode.${NC}"
$DRUSH state:set system.maintenance_mode 1

. $SCRIPTS_PATH/tasks/composer_install.sh

echo -e "${LIGHT_GREEN}Flush caches to be clean.${NC}"
$DRUSH cache:rebuild

echo -e "${LIGHT_GREEN}Launch updates. Ensure that the database schema is up-to-date.${NC}"
$DRUSH updatedb --entity-updates -y

. $SCRIPTS_PATH/tasks/update_config_split.sh

echo -e "${LIGHT_GREEN}Export global config split in case of overrides.${NC}"
$DRUSH config-split:export global -y

if [ "${ENVIRONMENT_MODE}" = "prod" ]; then
    echo -e "${LIGHT_GREEN}Export prod config split in case of overrides.${NC}"
    $DRUSH config-split:export prod -y
elif [ "${ENVIRONMENT_MODE}" = "staging" ]; then
    echo -e "${LIGHT_GREEN}Export staging config split in case of overrides.${NC}"
    $DRUSH config-split:export staging -y
else
    echo -e "${LIGHT_GREEN}Export dev config split in case of overrides.${NC}"
    $DRUSH config-split:export dev -y
fi

echo -e "${LIGHT_GREEN}Import configuration.${NC}"
$DRUSH config:import -y

. $SCRIPTS_PATH/tasks/update_translations.sh

echo -e "${LIGHT_GREEN}Remove the maintenance mode.${NC}"
$DRUSH state:set system.maintenance_mode 0

echo -e "${LIGHT_GREEN}Run CRON.${NC}"
$DRUSH core:cron

echo -e "${LIGHT_GREEN}Flush caches to be clean.${NC}"
$DRUSH cache:rebuild

echo -e "${LIGHT_GREEN}Back to the current directory.${NC}"
cd $CURRENT_PATH